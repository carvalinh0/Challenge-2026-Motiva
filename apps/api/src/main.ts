...
import { ListSensorsPageUseCase } from "./application/use-cases/sensor/ListSensorsPageUseCase";
import { GetReadingsSummaryUseCase } from "./application/use-cases/sensor/GetReadingsSummaryUseCase";
import { ResetSensorUseCase } from "./application/use-cases/sensor/ResetSensorUseCase";
import { GetSensorsHistoryUseCase } from "./application/use-cases/sensor/GetSensorsHistoryUseCase";
import { DeferSensorUseCase } from "./application/use-cases/sensor/DeferSensorUseCase";
...
import { GetRouteGeometryUseCase } from "./application/use-cases/route/GetRouteGeometryUseCase";
import { DefineRouteUseCase } from "./application/use-cases/route/DefineRouteUseCase";
import { CompleteRouteUseCase } from "./application/use-cases/route/CompleteRouteUseCase";
import { GetActiveRouteUseCase } from "./application/use-cases/route/GetActiveRouteUseCase";
...
if (!JWT_SECRET) {
  configErrors.push(
    "JWT_SECRET não configurado — impossível emitir ou validar tokens.",
  );
}
...
const controllers = {
  ...
  sensor: new SensorController(
    ...
    new ListSensorsUseCase(
      sensorRepository,
      measurementRepository,
      ACTIVE_WINDOW_MS,
    ),
    new ListSensorsPageUseCase(
      sensorRepository,
      measurementRepository,
      ACTIVE_WINDOW_MS,
    ),
    new GetReadingsSummaryUseCase(),
    new ResetSensorUseCase(sensorRepository, measurementRepository),
    new GetSensorsHistoryUseCase(sensorRepository, measurementRepository),
    new DeferSensorUseCase(sensorRepository),
  ),
  ...
  route: (() => {
    const routing = new OsrmRouteMatrixProvider();
    return new RouteController(
      new GetRouteMatrixUseCase(routing),
      new GetRouteGeometryUseCase(routing),
      new DefineRouteUseCase(),
      new CompleteRouteUseCase(),
      new GetActiveRouteUseCase(),
    );
  })(),
...
log.info(
  "startup",
  `DATABASE_URL: ${process.env.DATABASE_URL ?? "(nao configurado)"}`,
);
...
void sensorRepository
  ...
  .then((sensores) =>
    log.info(
      "startup",
      `Banco acessível — ${sensores.length} nó(s) cadastrado(s).`,
    ),
  )
  .catch((err) =>
    log.error(
