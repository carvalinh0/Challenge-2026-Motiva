...
export class ProxyController {
    ...
    create = async (c: Context) => {
        ...
        await this.registerProxy.execute(idParam(c), body);
        return ok(c, undefined, "Proxy criado com sucesso", 201);
    };

    get = async (c: Context) => ok(c, await this.getProxy.execute(idParam(c)));

    delete = async (c: Context) => {
        await this.deleteProxy.execute(idParam(c));
        return ok(c);
    };

    reset = async (c: Context) => {
        await this.resetProxy.execute(idParam(c));
        return ok(c);
    };
}
