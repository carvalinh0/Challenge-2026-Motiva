...
export const listSensorsQuerySchema = z.object({
  ...
  closeTo: z
    ...
    .regex(
      /^-?\d+(\.\d+)?,-?\d+(\.\d+)?$/,
      'closeTo deve ser "latitude,longitude"',
    )
    .optional(),
...
export const listSensorsPageQuerySchema = z.object({
...
export type ListSensorsPageQueryDTO = z.infer<
  typeof listSensorsPageQuerySchema
>;
...
export interface SensorPageOutputDTO {
  items: SensorSummaryDTO[];
  page: number;
  limit: number;
  total: number;
  totalPages: number;
...
export type ReadingsSummaryQueryDTO = z.infer<
  typeof readingsSummaryQuerySchema
>;

export interface ReadingsSummaryOutputDTO {
  bySensor: {
    name: string;
    Alto: number;
    Baixo: number;
...
export function toSensorSummaryDTO(
  ...
  return {
    ...
    type: sensor.type,
    createdAt: sensor.createdAt.getTime(),
    last_seen: sensor.lastSeen ? sensor.lastSeen.getTime() : null,
    active,
    deferred: sensor.deferredAt !== null,
    routeAssigned: sensor.routeId !== null,
