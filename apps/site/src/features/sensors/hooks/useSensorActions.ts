  ...
  const measure = useCallback(
    (sensor: SensorSummary) =>
      run(
        ...
        (value) => ({
          tone: "success",
          text: `${sensor.id} mediu: ${nivelFromValue(value)}.`,
        }),
      ),
    [run],
  );
    ...
    (sensor: SensorSummary) =>
      run(
        ...
        (result) => ({
          ...
          text: `${sensor.id} está ${result?.alive ? "vivo" : "sem resposta"}.`,
        }),
      ),
    [run],
  );
    ...
    if (action === "calibrate") {
      await run(
        ...
        (result) => ({
          ...
          text: `Calibração de ${sensor.id} ${result?.ok ? "concluída" : "falhou"}.`,
        }),
      );
      ...
      await run(
        ...
        () => ({ tone: "success", text: `"${sensor.id}" resetado.` }),
      );
    } else {
      await run(
        sensor.id,
        ...
        () => ({ tone: "success", text: `"${sensor.id}" excluído.` }),
      );
    }
  }, [confirmation, run]);
